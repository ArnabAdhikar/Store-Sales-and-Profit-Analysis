...
st.header("Store Sales and Profit Analysis")

# all graphs uses custom css not streamlit defaults
theme_plotly = None
...
# performs basic descriptive analytics like Mean,Mode,Sum  etc
def Home():
    with st.expander("VIEW EXCEL DATASET"):
        showData = st.multiselect('Filter: ', df_selection.columns,
                                  default=["Policy", "Expiry", "Location", "State", "Region", "Investment",
...
def graphs():
    ...
    fig_investment = px.bar(
        ...
        orientation="h",
        title="<b> INVESTMENT BASED ON BUSINESS TYPE </b>",
    ...
    fig_state = px.line(
        ...
        y="Investment",
        orientation="v",
        title="<b> INVESTMENT BASED ON STATE </b>",
    ...
    with center:
        # pie chart
        fig = px.pie(df_selection, values='Rating', names='State', title='RATINGS / REGIONS')
        fig.update_layout(legend_title="Regions", legend_y=0.9)
        fig.update_traces(textinfo='percent+label', textposition='inside')
        st.plotly_chart(fig, use_container_width=True, theme=theme_plotly)


# show current earnings against expected target
def Progressbar():
    st.markdown(
